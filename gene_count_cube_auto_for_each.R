...
sim_score = read.csv("/mnt/isilon/helbig_lab/Dropbox/galerp/HPO_testing/EGRP_EPGP_sim/EGRP_EPGP_v10/cube_sim_epgp_egrp_v10.csv",stringsAsFactors = F) %>% select(-X)
...
#     )
# 
# foo <- "/mnt/isilon/helbig_lab/Dropbox/galerp/HPO_testing/EGRP_EPGP_sim/EGRP_EPGP_v10/cube_sim_epgp_egrp_v10.csv"
# 
...
cube_sim_pat <- foreach( j = 1:length(pat_pairs)) %dopar% { 
  library(dplyr)
  sim_pat_draw(sim_score_file = "/mnt/isilon/helbig_lab/Dropbox/galerp/HPO_testing/EGRP_EPGP_sim/EGRP_EPGP_v10/cube_sim_epgp_egrp_v10.csv",task_id = j,permutation=100000)
  }
end_time <-  Sys.time()
end_time - start_time
names(cube_sim_pat) <- paste0("n_pats_",unlist(pat_pairs))
saveRDS(cube_sim_pat,"100k_cube_epgp-egrp_v10.rds")
...
exact_p_av2 <- function(sample_size,sim_score_med)
{
  ...
  p_value <- function(x){
    foo = ecdf(cube_sim_pat[[paste0("n_pats_",sample_size)]]$mean)
    return(1 - foo(x))
  }
...
exact_p_med2 <- function(sample_size,sim_score_med)
{
  #p_value inner-function to determine the significance of a value of x 
  p_value <- function(x) {
    foo = ecdf(cube_sim_pat[[paste0("n_pats_",sample_size)]]$median)
    return(1 - foo(x))
  }
  p <- p_value(sim_score_med)
  return(p)
...
exact_p_mod2 <- function(sample_size,sim_score_mod){
  ...
  #p_value inner-function to determine the significance of a value of x 
  p_value <- function(x) {
    foo = ecdf(cube_sim_pat[[paste0("n_pats_",sample_size)]]$mode)
    return(1 - foo(x))
  }
...
for (i in 1:nrow(gene_count)){
 ...
 write.csv(gene_count,"gene_count_cube_epgp_egrp_v10.csv",row.names = F)
