...
write.csv(sim_score,file = "sim_matrix.csv",row.names = T)
...
for (i in 1:nrow(gene_count)){
  ...
  p_average = exact_p(e1,gene_count$av_sim[i],input.yaml$num_of_draws)
  p_median = exact_p(e1,gene_count$median_sim[i],input.yaml$num_of_draws)
  p_mod = exact_p(e1,gene_count$mode_sim[i],input.yaml$num_of_draws)
